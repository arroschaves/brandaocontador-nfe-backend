...
function initializeAlertRules() {
  alertRules = [
    ...
    },
    {
      ...
      message: (metrics) =>
        `Alto uso de memória: ${Math.round(metrics.memoryUsagePercent * 100)}%`,
      throttle: 300000,
    },
    {
      ...
      message: (metrics) =>
        `Uso crítico de memória: ${Math.round(metrics.memoryUsagePercent * 100)}%`,
      throttle: 60000,
    },
    ...
    // Alertas de Performance
    {
      ...
      message: (metrics) =>
        `Alto lag do event loop: ${Math.round(metrics.eventLoopLag)}ms`,
    ...
    },
    {
      ...
      message: (metrics) =>
        `Tempo de resposta lento: ${Math.round(metrics.avgResponseTime)}ms`,
      throttle: 300000,
    },
    ...
    // Alertas de NFe
    {
      ...
      message: (metrics) =>
        `Alta taxa de erro NFe: ${Math.round(metrics.nfeErrorRate * 100)}%`,
    ...
    // Alertas de Segurança
    {
      ...
      message: (metrics) =>
        `Alta taxa de erro: ${Math.round(metrics.errorRate * 100)}%`,
      throttle: 300000,
    },
  ];
...
function processAlerts(metrics) {
  ...
  for (const rule of alertRules) {
    try {
      ...
      logger.error(`Erro ao processar regra de alerta ${rule.id}`, {
        error: error.message,
      });
    }
  }
...
function addAlertRule(rule) {
  if (!rule.id || !rule.condition || !rule.message) {
    throw new Error(
      "Regra de alerta inválida: id, condition e message são obrigatórios",
    );
  }
...
function initializeAlerts() {
  ...
  try {
    ...
    logger.error("Erro na inicialização do sistema de alertas", {
      error: error.message,
    });
