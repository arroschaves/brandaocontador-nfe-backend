...
class PerformanceMonitor {
  ...
  recordNFeEmission(success, responseTime) {
    ...
    const totalTime =
      this.metrics.nfe.tempoMedio * (this.metrics.nfe.emissoes - 1) +
      responseTime;
  ...
  checkAlerts() {
    ...
    const slowRequestRate =
      (this.metrics.requests.slowRequests / this.metrics.requests.total) * 100;
  ...
  async generateReport() {
    const report = {
      ...
      summary: {
        ...
        nfeSuccessRate: (
          (this.metrics.nfe.sucessos / this.metrics.nfe.emissoes) *
          100
        ).toFixed(2),
      },
      performance: {
