...
function startEventLoopMonitoring() {
  ...
  eventLoopMonitor = setInterval(
    measureEventLoopLag,
    monitoringConfig.apm.eventLoop.interval,
  );
}
...
async function collectSystemStats() {
  try {
    ...
    logger.error("Erro ao coletar estatísticas do sistema", {
      error: error.message,
    });
    return null;
  }
...
function performanceMiddleware(req, res, next) {
  ...
  res.end = function (...args) {
    ...
    const avgTime =
      apmStats.requests.responseTimeHistory.reduce((a, b) => a + b, 0) /
 ...
 */
function recordNfeProcessing(
  processingTime,
  success = true,
  sefazResponseTime = null,
) {
  if (success) {
  ...
  if (apmStats.nfe.processed > 0) {
    apmStats.nfe.avgProcessingTime =
      (apmStats.nfe.avgProcessingTime * (apmStats.nfe.processed - 1) +
        processingTime) /
      apmStats.nfe.processed;
  }
...
function generatePerformanceReport() {
  ...
  return {
    ...
    requests: {
      ...
      errorRate:
        apmStats.requests.total > 0
          ? `${Math.round((apmStats.requests.errors / apmStats.requests.total) * 100)}%`
          : "0%",
    ...
    system: {
      ...
      gc: {
        ...
        avgDuration:
          apmStats.system.gcStats.collections > 0
            ? `${Math.round(apmStats.system.gcStats.duration / apmStats.system.gcStats.collections)}ms`
            : "0ms",
      },
    },
    nfe: {
      ...
      errorRate:
        apmStats.nfe.processed + apmStats.nfe.errors > 0
          ? `${Math.round((apmStats.nfe.errors / (apmStats.nfe.processed + apmStats.nfe.errors)) * 100)}%`
          : "0%",
...
function performanceStatusHandler(req, res) {
  try {
    ...
    logger.error("Erro ao gerar relatório de performance", {
      error: error.message,
    });
    res.status(500).json({
