  ...
  try {
    ...
    if (req.user.tipo !== "admin") {
      advancedLogger.logWarning(
        "system",
        "Tentativa de acesso aos logs por usuário não autorizado",
        req,
        {
          userId: req.user.id,
    ...
    advancedLogger.logInfo(
      "system",
      "Solicitação de visualização de logs",
      req,
      {
        userId: req.user.id,
        categoria,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      advancedLogger.logInfo(
        "system",
        "Solicitação de estatísticas de logs",
        req,
        {
          userId: req.user.id,
          dias,
      ...
      advancedLogger.logError(
        "system",
        "Erro ao obter estatísticas de logs",
        req,
        error,
        {
          userId: req.user?.id,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      advancedLogger.logInfo(
        "system",
        "Solicitação de dashboard de logs",
        req,
        {
          userId: req.user.id,
        },
      ...
      advancedLogger.logError(
        "system",
        "Erro ao obter dashboard de logs",
        req,
        error,
        {
          userId: req.user?.id,
