...
class SecurityMiddleware {
  ...
  analisarRequisicao(req) {
    ...
    const botPatterns = [/sqlmap/i, /nikto/i, /nessus/i, /openvas/i, /nmap/i];
  ...
  isIPConfiavel(ip) {
    const ipsConfiáveis = ["127.0.0.1", "::1", "localhost"];
    return ipsConfiáveis.includes(ip);
  }

  marcarIPSuspeito(ip, motivo = "rate_limit") {
    const agora = Date.now();
    const suspeita = this.suspiciousIPs.get(ip) || {
      count: 0,
      firstSeen: agora,
      motivos: [],
    };
  ...
  logRateLimitExcedido(req, tipo) {
    const ip = this.getClientIP(req);
    console.warn(
      `⚠️ RATE LIMIT: ${tipo} excedido para IP ${ip} - ${req.method} ${req.originalUrl}`,
    );
  }
  ...
  logSlowDownAtivado(req) {
    const ip = this.getClientIP(req);
    console.warn(
      `🐌 SLOW DOWN: Ativado para IP ${ip} - ${req.method} ${req.originalUrl}`,
    );
  }
  ...
  logTentativaSQLInjection(req, key) {
    const ip = this.getClientIP(req);
    console.warn(
      `💉 SQL INJECTION: Tentativa detectada de IP ${ip} - Campo: ${key}`,
    );
  }
  ...
  logTentativaCSRF(req) {
    const ip = this.getClientIP(req);
    console.warn(
      `🛡️ CSRF: Tentativa de ataque de IP ${ip} - ${req.method} ${req.originalUrl}`,
    );
  }
  ...
  logAcessoBloqueado(req) {
    const ip = this.getClientIP(req);
    console.warn(
      `🚫 ACESSO BLOQUEADO: IP ${ip} - ${req.method} ${req.originalUrl}`,
    );
  }
  ...
  logAmeacasDetectadas(req, ameacas) {
    const ip = this.getClientIP(req);
    console.warn(
      `⚠️ AMEAÇAS: ${ameacas.join(", ")} detectadas de IP ${ip} - ${req.originalUrl}`,
    );
  }
...
module.exports = {
  responseTime,
  configurarHelmet:
    securityMiddleware.configurarHelmet.bind(securityMiddleware),
  configurarCORS: securityMiddleware.configurarCORS.bind(securityMiddleware),
  configurarRateLimitingGlobal:
    securityMiddleware.configurarRateLimitingGlobal.bind(securityMiddleware),
  configurarRateLimitingAuth:
    securityMiddleware.configurarRateLimitingAuth.bind(securityMiddleware),
  configurarRateLimitingAPI:
    securityMiddleware.configurarRateLimitingAPI.bind(securityMiddleware),
  configurarSlowDown:
    securityMiddleware.configurarSlowDown.bind(securityMiddleware),
  configurarSanitizacao:
    securityMiddleware.configurarSanitizacao.bind(securityMiddleware),
  detectarAmeacas: securityMiddleware.detectarAmeacas.bind(securityMiddleware),
  obterStatusSeguranca:
    securityMiddleware.obterStatusSeguranca.bind(securityMiddleware),
};
