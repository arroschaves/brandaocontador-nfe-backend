...
class TaxCalculationService {
    constructor() {
        ...
        this.simplesNacionalAnexos = {
            I: [ // Comércio
                { faixa: 180000, aliquota: 4.00, deducao: 0 },
                { faixa: 360000, aliquota: 7.30, deducao: 5940 },
                { faixa: 720000, aliquota: 9.50, deducao: 13860 },
                { faixa: 1800000, aliquota: 10.70, deducao: 22500 },
                { faixa: 3600000, aliquota: 14.30, deducao: 87300 },
                { faixa: 4800000, aliquota: 19.00, deducao: 378000 }
            ],
            II: [ // Indústria
                { faixa: 180000, aliquota: 4.50, deducao: 0 },
                { faixa: 360000, aliquota: 7.80, deducao: 5940 },
                { faixa: 720000, aliquota: 10.00, deducao: 13860 },
                { faixa: 1800000, aliquota: 11.20, deducao: 22500 },
                { faixa: 3600000, aliquota: 14.70, deducao: 85500 },
                { faixa: 4800000, aliquota: 30.00, deducao: 720000 }
            ],
            III: [ // Serviços
                { faixa: 180000, aliquota: 6.00, deducao: 0 },
                { faixa: 360000, aliquota: 11.20, deducao: 9360 },
                { faixa: 720000, aliquota: 13.50, deducao: 17640 },
                { faixa: 1800000, aliquota: 16.00, deducao: 35640 },
                { faixa: 3600000, aliquota: 21.00, deducao: 125640 },
                { faixa: 4800000, aliquota: 33.00, deducao: 648000 }
            ]
        };
    ...
    calcularSimplesNacional(faturamento12Meses, anexo, valorVenda) {
        ...
        const aliquotaEfetiva = ((faturamento12Meses * faixaEncontrada.aliquota / 100) - faixaEncontrada.deducao) / faturamento12Meses * 100;
        ...
        return {
            ...
            detalhamento: this.detalharSimplesNacional(anexo, aliquotaEfetiva, valorVenda)
        };
    }
    ...
    detalharSimplesNacional(anexo, aliquotaEfetiva, valorVenda) {
        ...
        const distribuicao = {
            I: { // Comércio
                irpj: 5.50, csll: 3.50, cofins: 12.74, pis: 2.76,
                cpp: 41.50, icms: 34.00
            },
            II: { // Indústria  
                irpj: 5.50, csll: 3.50, cofins: 12.74, pis: 2.76,
                cpp: 41.50, icms: 32.00, ipi: 2.00
            },
            III: { // Serviços
                irpj: 4.00, csll: 3.50, cofins: 12.82, pis: 2.78,
                cpp: 43.40, iss: 33.50
            }
        };
        ...
        for (const [imposto, percentual] of Object.entries(dist)) {
            detalhes[imposto] = parseFloat((valorTotal * percentual / 100).toFixed(2));
        }
    ...
    calcularICMS(valorProduto, ufOrigem, ufDestino, cst, aliquotaReduzida = null) {
        let aliquota = 0;
    ...
    calcularAliquotaInterestadual(ufOrigem, ufDestino) {
        ...
        if (regioesSulSudeste.includes(ufOrigem) && regioesSulSudeste.includes(ufDestino)) {
    ...
    calcularImpostosCompleto(produto, configuracao) {
        ...
        try {
            ...
            if (regimeTributario === 'simplesNacional') {
                ...
                resultado.totalImpostos += resultado.impostos.simplesNacional.valorImpostos;
            } else {
            ...
            resultado.valorLiquido = parseFloat((valorTotal - resultado.totalImpostos).toFixed(2));
        ...
        for (const produto of produtos) {
            ...
            for (const [tipo, imposto] of Object.entries(calculo.impostos)) {
                ...
                if (tipo === 'simplesNacional') {
                    ...
                    relatorio.resumo.impostosPorTipo.pis = (relatorio.resumo.impostosPorTipo.pis || 0) + imposto.pis.valor;
                    relatorio.resumo.impostosPorTipo.cofins = (relatorio.resumo.impostosPorTipo.cofins || 0) + imposto.cofins.valor;
                }
            }
        }

        // Arredondar valores do resumo
        relatorio.resumo.valorTotalProdutos = parseFloat(relatorio.resumo.valorTotalProdutos.toFixed(2));
        relatorio.resumo.totalImpostos = parseFloat(relatorio.resumo.totalImpostos.toFixed(2));
        relatorio.resumo.valorLiquido = parseFloat(relatorio.resumo.valorLiquido.toFixed(2));

        for (const [tipo, valor] of Object.entries(relatorio.resumo.impostosPorTipo)) {
