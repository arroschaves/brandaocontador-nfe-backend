...
class ProductionStarter {
  ...
  async verificarPorta() {
    return new Promise((resolve, reject) => {
      ...
      server.on('error', (err) => {
        reject(new Error(`Erro ao verificar porta ${this.config.port}: ${err.message}`));
      });
    });
  }
  ...
  async iniciarMaster() {
    ...
    console.log(`🔧 Configuração: ${this.config.workers} workers, porta ${this.config.port}`);
    ...
    cluster.on('listening', (worker, address) => {
      console.log(`🎧 Worker ${worker.process.pid} escutando em ${address.address}:${address.port}`);
    });
  ...
  criarWorker() {
    ...
    const timeout = setTimeout(() => {
      console.error(`⏰ Worker ${worker.process.pid} não respondeu, matando...`);
      worker.kill();
    }, 30000);
    ...
    if (this.config.maxMemory) {
      ...
      setInterval(() => {
        ...
        if (usage.heapUsed > maxMemoryBytes) {
          console.warn(`⚠️  Worker ${process.pid} excedeu limite de memória, reiniciando...`);
          process.exit(0);
        }
      }, 60000);
    }
  ...
  iniciarMonitoramento() {
    ...
    setInterval(() => {
      ...
      console.log(`📈 Master - Memória: ${this.formatBytes(usage.heapUsed)}, Workers: ${this.workers.size}`);
