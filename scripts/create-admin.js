...
async function createAdmin() {
  ...
  console.log(
    "║    CRIAÇÃO DE USUÁRIO ADMINISTRADOR - Sistema NFe Brandão     ║",
  );
  console.log(
    "╚════════════════════════════════════════════════════════════════╝\n",
  );
  ...
  try {
    ...
    if (fs.existsSync(usersPath)) {
      ...
      try {
        ...
        console.log(
          `📋 Arquivo users.json encontrado com ${users.length} usuário(s)`,
        );
      } catch (error) {
    ...
    if (adminExists) {
      ...
      return new Promise((resolve) => {
        readline.question(
          "\n❓ Deseja SUBSTITUIR o usuário existente? (sim/não): ",
          (answer) => {
            readline.close();
            if (
