...
async def run_test():
    ...
    try:
        ...
        # -> Retry clicking the 'Entrar' button to login or try alternative method to submit the form.
        frame = context.pages[-1]
        # Retry clicking the Entrar button to login
        elem = frame.locator('xpath=html/body/div/div/div[2]/div[2]/div/div/div/div[2]/form/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input valid email and password into the respective fields and then click the 'Entrar' button to login.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Vercel deployment failed due to errors').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Frontend automated deploy via Vercel did not trigger or complete successfully on push to main branch.")
