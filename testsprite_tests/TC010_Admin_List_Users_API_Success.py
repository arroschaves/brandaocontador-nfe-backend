...
async def run_test():
    ...
    try:
        ...
        # -> Retry login or check for any issues preventing login success.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=User list retrieval successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The administrative API did not return the user list correctly or the operation was not logged as expected.")
