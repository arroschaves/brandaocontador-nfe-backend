...
class ClienteService {
  ...
  async criarCliente(dadosCliente, usuarioId) {
    try {
      ...
      const validacao =
        await this.validationService.validarEEnriquecerCliente(dadosCliente);
  ...
  async atualizarCliente(clienteId, dadosAtualizacao, usuarioId) {
    try {
      ...
      if (dadosAtualizacao.documento || dadosAtualizacao.email) {
        const validacao =
          await this.validationService.validarEEnriquecerCliente({
            ...cliente,
            ...dadosAtualizacao,
          });
      ...
      const clienteAtualizado = await this.db.atualizarCliente(
        clienteId,
        dadosAtualizados,
      );
  ...
  async listarClientes(filtros = {}, usuarioId) {
    try {
      ...
      let clientesFiltrados = clientes.filter(
        (c) => c.usuarioId === usuarioId && c.ativo,
      );
      ...
      if (filtros.tipoCliente) {
        clientesFiltrados = clientesFiltrados.filter(
          (c) => c.tipoCliente === filtros.tipoCliente,
        );
      }
      ...
      if (filtros.uf) {
        clientesFiltrados = clientesFiltrados.filter(
          (c) => c.endereco?.uf === filtros.uf.toUpperCase(),
        );
      }
  ...
  async obterEstatisticas(usuarioId) {
    try {
      const clientes = await this.db.listarClientes();
      const clientesUsuario = clientes.filter(
        (c) => c.usuarioId === usuarioId && c.ativo,
      );
