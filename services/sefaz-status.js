...
class SefazStatusService {
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        if (attempt < this.retryAttempts) {
          console.warn(
            `⚠️ Tentativa ${attempt} falhou para ${uf}, tentando novamente em ${this.retryDelay}ms...`,
          );
  ...
  async _verificarStatusServico(uf, ambiente) {
    try {
      ...
      const operations = Object.keys(client).filter(
        (key) =>
  ...
  async verificarStatusGeral(ambiente = "1") {
    ...
    const statusGeral = await this.verificarStatusMultiplasUFs(
      todasUFs,
      ambiente,
    );
    ...
    return {
      ...
      sistemaGeral: {
        status:
          statusGeral.resumo.percentualDisponibilidade >= 80
            ? "online"
            : "instavel",
