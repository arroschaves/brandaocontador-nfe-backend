...
class UserService {
  ...
  async obterEstatisticasUsuario(userId) {
    try {
      ...
      const valorTotal = nfes.reduce((total, nfe) => total + (nfe.valorTotal || 0), 0);
      ...
      return {
        ...
        data: {
          ...
          diasAtivo: Math.ceil((new Date() - new Date(user.criadoEm)) / (1000 * 60 * 60 * 24))
        }
      };
      
    } catch (error) {
      console.error('❌ Erro ao obter estatísticas:', error.message);
      throw new Error(`Falha ao obter estatísticas do usuário: ${error.message}`);
    }
  }
  ...
  async listarUsuarios(filtros = {}) {
    try {
      ...
      if (filtros.ativo !== undefined) {
        usuariosFiltrados = usuariosFiltrados.filter(user => user.ativo === filtros.ativo);
      }

      if (filtros.empresa) {
        usuariosFiltrados = usuariosFiltrados.filter(user => 
          user.empresa && user.empresa.toLowerCase().includes(filtros.empresa.toLowerCase())
        );
      }

      // Remove dados sensíveis
      usuariosFiltrados = usuariosFiltrados.map(({ senha, ...userData }) => userData);
