...
class AdvancedLogger {
    ...
    ensureLogDirectories() {
        const categories = ['auth', 'nfe', 'clientes', 'certificados', 'dashboard', 'produtos', 'system', 'cte', 'mdfe', 'eventos', 'relatorios'];
        ...
        const specialDirs = ['uncaught-exceptions', 'unhandled-rejections', 'critical-errors'];
    ...
    getLogFileName(category, type = 'errors', subType = null) {
        ...
        if (subType) {
            return path.join(this.logsDir, category, type, `${today}_${hour}h_${subType}.json`);
        }
    ...
    detectCategory(req) {
        ...
        if (url.includes('/api/auth') || url.includes('/login') || url.includes('/logout')) {
            return 'auth';
        } else if (url.includes('/api/nfe')) {
            return 'nfe';
        } else if (url.includes('/api/clientes')) {
            return 'clientes';
        } else if (url.includes('/api/me/certificado') || url.includes('/certificado')) {
            return 'certificados';
    ...
    formatLogEntry(level, message, req = null, error = null, additionalData = {}) {
        const now = new Date();
    ...
    sanitizeBody(body) {
        ...
        const sensitiveFields = ['password', 'senha', 'token', 'secret', 'key', 'apiKey'];
    ...
    writeLog(category, level, message, req = null, error = null, additionalData = {}) {
        try {
            const logEntry = this.formatLogEntry(level, message, req, error, additionalData);
            ...
            if (level === 'ERROR' && error && (error.statusCode >= 500 || error.name === 'UnhandledPromiseRejectionWarning')) {
                const criticalFileName = this.getLogFileName('system', 'critical-errors', error.name || 'unknown');
            ...
            try {
                ...
                fs.appendFileSync(emergencyFile, JSON.stringify(emergencyLog, null, 2) + '\n');
            } catch (emergencyError) {
                console.error('Erro crítico ao escrever log de emergência:', emergencyError);
            }
        }
    }
    ...
    logRequest(req, res, next) {
        ...
        // Log da requisição
        this.writeLog(category, 'INFO', `${req.method} ${req.originalUrl || req.url}`, req);
        ...
        res.send = function(data) {
            ...
            if (res.statusCode >= 400) {
                ...
                logger.writeLog(category, 'ERROR', `${req.method} ${req.originalUrl || req.url} - Status: ${res.statusCode}`, req, null, {
                ...
                const logger = req.app.get('advancedLogger');
                logger.writeLog(category, 'INFO', `${req.method} ${req.originalUrl || req.url} - Status: ${res.statusCode}`, req, null, {
                    statusCode: res.statusCode,
                    duration
                });
            }
    ...
    getLogs(category, type = 'errors', date = null) {
        try {
            ...
            const fileName = path.join(this.logsDir, category, `${targetDate}-${type}.log`);
    ...
    setupGlobalErrorHandlers() {
        ...
        process.on('uncaughtException', (error) => {
            ...
            const fileName = this.getLogFileName('system', 'uncaught-exceptions', 'critical');
            const dir = path.dirname(fileName);
            if (!fs.existsSync(dir)) {
                fs.mkdirSync(dir, { recursive: true });
            }
            
            const logEntry = this.formatLogEntry('CRITICAL', 'Uncaught Exception', null, error, {
                type: 'uncaughtException',
        ...
        process.on('unhandledRejection', (reason, promise) => {
            ...
            // Salvar em arquivo especial
            const fileName = this.getLogFileName('system', 'unhandled-rejections', 'critical');
            const dir = path.dirname(fileName);
            if (!fs.existsSync(dir)) {
                fs.mkdirSync(dir, { recursive: true });
            }
            
            const logEntry = this.formatLogEntry('CRITICAL', 'Unhandled Promise Rejection', null, error, {
                type: 'unhandledRejection',
                reason: reason
            });
    ...
    globalErrorHandler() {
        return (error, req, res, next) => {
            ...
            this.logError(category, 'Erro capturado pelo middleware global', req, error, {
                type: 'expressError',
