...
class DanfeService {
  ...
  async gerarDanfe(dadosNfe, xmlAssinado) {
    try {
      ...
      await this.gerarIdentificacaoNfe(doc, dadosNfe, chaveAcesso, qrCodeDataURL);
  ...
  gerarChaveAcesso(dadosNfe) {
    try {
      ...
      const cnpj = (dadosNfe.emitente?.cnpj || '').replace(/\D/g, '').padStart(14, '0');
      const modelo = '55'; // NFe
      ...
      const chaveSemDv = uf + aamm + cnpj + modelo + serie + numero + tipoEmissao + codigo;
  ...
  async gerarCabecalhoOficial(doc, dadosNfe) {
    ...
    doc.fontSize(12)
       .font(this.config.fonts.bold)
       .text(`${dadosNfe.tipoOperacao || '1'} - ${tipoOperacao}`, 350, y, { align: 'center' });
  ...
  async gerarIdentificacaoNfe(doc, dadosNfe, chaveAcesso, qrCodeDataURL) {
    ...
    if (dadosNfe.protocolo) {
      ...
      doc.fontSize(9)
         .font(this.config.fonts.normal)
         .text(`${dadosNfe.protocolo} - ${dadosNfe.dataAutorizacao || new Date().toLocaleString()}`, 50, y + 47);
    }
  ...
  async gerarDadosEmitente(doc, emitente) {
    ...
    doc.fontSize(8)
       .font(this.config.fonts.normal)
       .text(`${emitente.endereco?.logradouro || ''}, ${emitente.endereco?.numero || ''}`, xDados, y + 25)
       .text(`${emitente.endereco?.complemento || ''}`, xDados, y + 35)
       .text(`${emitente.endereco?.bairro || ''} - ${emitente.endereco?.municipio || ''} - ${emitente.endereco?.uf || ''}`, xDados, y + 45)
       .text(`CEP: ${this.formatarCep(emitente.endereco?.cep)} - Fone: ${emitente.telefone || ''}`, xDados, y + 55);
    
    // CNPJ/IE - campos obrigatórios
    doc.fontSize(8)
       .text(`CNPJ: ${this.formatarCnpj(emitente.cnpj)}`, xDados, y + 70)
       .text(`IE: ${emitente.inscricaoEstadual || 'ISENTO'}`, xDados + 120, y + 70);
    ...
    if (emitente.cnae) {
      doc.fontSize(7)
         .text(`CNAE: ${emitente.cnae}`, xDados, y + 82);
    }
  ...
  async gerarDadosDestinatario(doc, destinatario) {
    ...
    doc.fontSize(8)
       .font(this.config.fonts.normal)
       .text(`Nome/Razão Social: ${destinatario.nome || destinatario.razaoSocial || ''}`, 55, y + 12);
    
    // CNPJ/CPF e Data de Emissão
    const documento = this.formatarDocumento(destinatario.cnpj || destinatario.cpf);
    const dataEmissao = this.formatarDataHora(new Date());
    
    doc.text(`CNPJ/CPF: ${documento}`, 55, y + 25)
       .text(`Data de Emissão: ${dataEmissao}`, 300, y + 25);
    
    // Data de Saída/Entrada e Hora
    doc.text(`Data de Saída/Entrada: ${this.formatarData(new Date())}`, 55, y + 38)
       .text(`Hora de Saída/Entrada: ${this.formatarHora(new Date())}`, 300, y + 38);
    
    // Endereço completo
    if (destinatario.endereco) {
      doc.text(`Endereço: ${destinatario.endereco.logradouro || ''}, ${destinatario.endereco.numero || ''}`, 55, y + 51)
         .text(`Bairro/Distrito: ${destinatario.endereco.bairro || ''}`, 55, y + 64)
         .text(`Município: ${destinatario.endereco.municipio || ''}`, 200, y + 64)
         .text(`Fone/Fax: ${destinatario.telefone || ''}`, 350, y + 64)
         .text(`UF: ${destinatario.endereco.uf || ''}`, 450, y + 64)
         .text(`CEP: ${this.formatarCep(destinatario.endereco.cep)}`, 480, y + 64);
  ...
  async gerarDadosEntrega(doc, entrega) {
    ...
    doc.fontSize(8)
       ...
       .text(`CNPJ/CPF: ${this.formatarDocumento(entrega.cnpj || entrega.cpf)}`, 55, y + 25)
       .text(`Endereço: ${entrega.endereco.logradouro}, ${entrega.endereco.numero}`, 55, y + 38)
  ...
  async gerarItensDetalhados(doc, itens) {
    ...
    itens.forEach((item, index) => {
      ...
      doc.fontSize(6)
         ...
         .text(this.formatarQuantidade(item.quantidade), 385, yAtual, { width: 25, align: 'right' })
         .text(this.formatarMoeda(item.valorUnitario), 415, yAtual, { width: 35, align: 'right' })
         .text(this.formatarMoeda(item.valorTotal), 455, yAtual, { width: 35, align: 'right' })
         .text(this.formatarMoeda(item.baseCalculoIcms), 495, yAtual, { width: 25, align: 'right' })
         .text(this.formatarMoeda(item.valorIcms), 525, yAtual, { width: 20, align: 'right' });
  ...
  async gerarCalculoImposto(doc, totais) {
    ...
    doc.fontSize(7)
       ...
       .text(this.formatarMoeda(totais?.baseCalculoIcms || 0), 55, y + 25, { align: 'center', width: 80 });
    
    // Valor do ICMS
    doc.text('VALOR DO ICMS', 140, y + 15)
       .text(this.formatarMoeda(totais?.valorIcms || 0), 140, y + 25, { align: 'center', width: 80 });
    
    // Base de cálculo ICMS ST
    doc.text('BASE DE CÁLCULO DO ICMS ST', 225, y + 15)
       .text(this.formatarMoeda(totais?.baseCalculoIcmsSt || 0), 225, y + 25, { align: 'center', width: 80 });
    
    // Valor ICMS ST
    doc.text('VALOR DO ICMS ST', 310, y + 15)
       .text(this.formatarMoeda(totais?.valorIcmsSt || 0), 310, y + 25, { align: 'center', width: 80 });
    
    // Valor total dos produtos
    doc.text('VALOR TOTAL DOS PRODUTOS', 395, y + 15)
       .text(this.formatarMoeda(totais?.valorProdutos || 0), 395, y + 25, { align: 'center', width: 80 });
    
    // Segunda linha de totais
    doc.text('VALOR DO FRETE', 55, y + 45)
       .text(this.formatarMoeda(totais?.valorFrete || 0), 55, y + 55, { align: 'center', width: 80 });
    
    doc.text('VALOR DO SEGURO', 140, y + 45)
       .text(this.formatarMoeda(totais?.valorSeguro || 0), 140, y + 55, { align: 'center', width: 80 });
    
    doc.text('DESCONTO', 225, y + 45)
       .text(this.formatarMoeda(totais?.valorDesconto || 0), 225, y + 55, { align: 'center', width: 80 });
    
    doc.text('OUTRAS DESPESAS', 310, y + 45)
       .text(this.formatarMoeda(totais?.outrasDespesas || 0), 310, y + 55, { align: 'center', width: 80 });
    
    doc.text('VALOR DO IPI', 395, y + 45)
       .text(this.formatarMoeda(totais?.valorIpi || 0), 395, y + 55, { align: 'center', width: 80 });
    
    // Valor total da nota
    doc.fontSize(9)
       .font(this.config.fonts.bold)
       .text('VALOR TOTAL DA NOTA', 480, y + 45)
       .text(this.formatarMoeda(totais?.valorTotal || 0), 480, y + 55, { align: 'center', width: 80 });
    ...
    for (let i = 1; i < 6; i++) {
      const x = 50 + (i * 85);
      doc.moveTo(x, y).lineTo(x, y + 75).stroke();
    }
    
    // Separador horizontal
    doc.moveTo(50, y + 40).lineTo(545, y + 40).stroke();
  ...
  async gerarTransporteVolumes(doc, transporte) {
    ...
    if (transporte && transporte.transportador) {
      ...
      doc.fontSize(7)
         ...
         .text(`Frete por conta: ${this.obterModalidadeFrete(transporte.modalidadeFrete)}`, 55, y + 25)
         .text(`CNPJ/CPF: ${this.formatarDocumento(transp.cnpj || transp.cpf)}`, 200, y + 12)
    ...
    if (transporte && transporte.volumes) {
      doc.fontSize(7)
         ...
         .text(`Peso Líquido: ${transporte.volumes.pesoLiquido || 0} kg`, 350, y + 52)
         .text(`Peso Bruto: ${transporte.volumes.pesoBruto || 0} kg`, 450, y + 52);
    }
  ...
  async gerarDadosAdicionais(doc, dadosNfe) {
    ...
    // Separador vertical
    doc.moveTo(300, y).lineTo(300, y + 65).stroke();
  ...
  async gerarDadosNfe(doc, dadosNfe) {
    ...
    // Informações fiscais
    doc.fontSize(8)
       .text(`Natureza da Operação: ${dadosNfe.naturezaOperacao || ''}`, 50, y + 80)
       .text(`Protocolo de Autorização: ${dadosNfe.protocolo || ''}`, 300, y + 80)
       .text(`Data/Hora de Autorização: ${this.formatarDataHora(new Date())}`, 300, y + 95);
  ...
  async gerarItens(doc, itens) {
    ...
    itens.forEach((item, index) => {
      ...
      doc.fontSize(7)
         ...
         .text(item.impostos?.icms?.cst || '', 360, yAtual, { width: 30, align: 'center' })
         .text(item.cfop || '', 395, yAtual, { width: 35, align: 'center' })
         .text(item.unidade || '', 435, yAtual, { width: 25, align: 'center' })
         .text(this.formatarNumero(item.quantidade), 465, yAtual, { width: 35, align: 'right' })
         .text(this.formatarMoeda(item.valorUnitario), 505, yAtual, { width: 40, align: 'right' })
         .text(this.formatarMoeda(item.valorTotal), 550, yAtual, { width: 45, align: 'right' });
  ...
  async gerarTotais(doc, totais) {
    ...
    doc.fontSize(8)
       ...
       .text(`Base Cálc. ICMS ST: ${this.formatarMoeda(totais?.vBCST || '0.00')}`, 245, y + 15)
       .text(`Valor ICMS ST: ${this.formatarMoeda(totais?.vST || '0.00')}`, 350, y + 15)
       .text(`Valor Total Produtos: ${this.formatarMoeda(totais?.vProd)}`, 450, y + 15);
    
    // Segunda linha de totais
    doc.text(`Valor Frete: ${this.formatarMoeda('0.00')}`, 55, y + 30)
       ...
       .text(`Valor IPI: ${this.formatarMoeda(totais?.vIPI || '0.00')}`, 450, y + 30);
    
    // Total da NFe
    doc.fontSize(10)
       .font(this.config.fonts.bold)
       .text(`VALOR TOTAL DA NFe: ${this.formatarMoeda(totais?.vNF)}`, 350, y + 50);
    
    // Impostos aproximados (Lei da Transparência)
    if (totais?.vTotTrib && parseFloat(totais.vTotTrib) > 0) {
      doc.fontSize(8)
         .font(this.config.fonts.normal)
         .text(`Valor Aproximado dos Tributos: ${this.formatarMoeda(totais.vTotTrib)} (Lei 12.741/2012)`, 55, y + 70);
    }
  ...
  async gerarTransporte(doc, transporte) {
    ...
    if (transporte) {
      doc.fontSize(8)
         ...
         .text(`CNPJ/CPF: ${this.formatarDocumento(transporte.cnpj || transporte.cpf)}`, 300, y + 15)
  ...
  async gerarRodape(doc, dadosNfe) {
    ...
    doc.fontSize(6)
       ...
       .text('Este documento foi emitido por sistema próprio conforme legislação vigente.', 50, y, { align: 'center' })
       .text(`Emitido em: ${this.formatarDataHora(new Date())}`, 50, y + 10, { align: 'center' });
  }
