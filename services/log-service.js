...
class LogService {
  ...
  async obterEstatisticas() {
    try {
      ...
      const logsHoje = await this.lerLogs(
        this.LOG_FILE,
        inicioHoje.toISOString(),
      );
      const logsSemana = await this.lerLogs(
        this.LOG_FILE,
        inicioSemana.toISOString(),
      );
      const errosHoje = await this.lerLogs(
        this.ERROR_FILE,
        inicioHoje.toISOString(),
      );
      ...
      return {
        ...
        sistema: {
          ...
          ultimaOperacao:
            logsHoje.length > 0
              ? logsHoje[logsHoje.length - 1].timestamp
              : null,
        },
      };
    } catch (error) {
  ...
  sanitizarDados(dados) {
    ...
    Object.keys(dadosLimpos).forEach((chave) => {
      if (
        typeof dadosLimpos[chave] === "string" &&
        dadosLimpos[chave].length > 1000
      ) {
        dadosLimpos[chave] =
          dadosLimpos[chave].substring(0, 1000) + "... [TRUNCADO]";
      }
    });
