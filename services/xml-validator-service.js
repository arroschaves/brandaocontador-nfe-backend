...
class XmlValidatorService {
  ...
  validarEmitente(emitente) {
    ...
    if (
      ...
      ![1, 2, 3].includes(emitente.regimeTributario)
    ) {
  ...
  validarEndereco(endereco, tipo) {
    ...
    if (
      ...
      !/^\d{7}$/.test(endereco.codigoMunicipio)
    ) {
  ...
  validarItens(itens) {
    ...
    itens.forEach((item, index) => {
      ...
      if (!item.codigo || item.codigo.trim().length === 0) {
        this.erros.push(
          `Código do produto é obrigatório no item ${numeroItem}`,
        );
      }

      if (!item.descricao || item.descricao.trim().length === 0) {
        this.erros.push(
          `Descrição do produto é obrigatória no item ${numeroItem}`,
        );
      }
      ...
      if (!item.quantidade || item.quantidade <= 0) {
        this.erros.push(
          `Quantidade deve ser maior que zero no item ${numeroItem}`,
        );
      }

      if (!item.valorUnitario || item.valorUnitario <= 0) {
        this.erros.push(
          `Valor unitário deve ser maior que zero no item ${numeroItem}`,
        );
      }
  ...
  validarTributacaoItem(item, numeroItem) {
    ...
    if (tributacao.icms) {
      if (!this.cstsICMS.includes(tributacao.icms.cst)) {
        this.erros.push(
          `CST do ICMS inválido no item ${numeroItem}: ${tributacao.icms.cst}`,
        );
      }
      ...
      if (tributacao.icms.valor > 0) {
        if (!tributacao.icms.baseCalculo || tributacao.icms.baseCalculo <= 0) {
          this.erros.push(
            `Base de cálculo do ICMS obrigatória quando há valor no item ${numeroItem}`,
          );
        }
    ...
    if (tributacao.ipi) {
      if (!this.cstsIPI.includes(tributacao.ipi.cst)) {
        this.erros.push(
          `CST do IPI inválido no item ${numeroItem}: ${tributacao.ipi.cst}`,
        );
      }
    }
    ...
    if (tributacao.pis) {
      if (!this.cstsPIS.includes(tributacao.pis.cst)) {
        this.erros.push(
          `CST do PIS inválido no item ${numeroItem}: ${tributacao.pis.cst}`,
        );
      }
    }
    ...
    if (tributacao.cofins) {
      if (!this.cstsCOFINS.includes(tributacao.cofins.cst)) {
        this.erros.push(
          `CST do COFINS inválido no item ${numeroItem}: ${tributacao.cofins.cst}`,
        );
      }
    }
  }
  ...
  validarNfe40(dados) {
    ...
    if (dados.destinatario) {
      if (dados.destinatario.cnpj) {
        ...
        if (
          !dados.destinatario.indicadorIE ||
          ![1, 2, 9].includes(dados.destinatario.indicadorIE)
        ) {
          this.avisos.push(
            "Indicador de IE do destinatário deve ser informado para CNPJ",
          );
        }
        ...
        if (dados.destinatario.indicadorIE !== 9) {
          this.avisos.push(
            "Para CPF, indicador de IE deve ser 9 (Não Contribuinte)",
          );
        }
      }
    }
    ...
    if (
      !dados.indicadorPresenca ||
      ![0, 1, 2, 3, 4, 5, 9].includes(dados.indicadorPresenca)
    ) {
  ...
  validarTributacao(dados) {
    ...
    if (dados.totais) {
      ...
      if (Math.abs((dados.totais.vICMS || 0) - totalICMS) > tolerancia) {
        this.erros.push(
          `Total de ICMS divergente. Calculado: ${totalICMS.toFixed(2)}, Informado: ${(dados.totais.vICMS || 0).toFixed(2)}`,
        );
      }

      if (Math.abs((dados.totais.vIPI || 0) - totalIPI) > tolerancia) {
        this.erros.push(
          `Total de IPI divergente. Calculado: ${totalIPI.toFixed(2)}, Informado: ${(dados.totais.vIPI || 0).toFixed(2)}`,
        );
      }

      if (Math.abs((dados.totais.vPIS || 0) - totalPIS) > tolerancia) {
        this.erros.push(
          `Total de PIS divergente. Calculado: ${totalPIS.toFixed(2)}, Informado: ${(dados.totais.vPIS || 0).toFixed(2)}`,
        );
      }

      if (Math.abs((dados.totais.vCOFINS || 0) - totalCOFINS) > tolerancia) {
        this.erros.push(
          `Total de COFINS divergente. Calculado: ${totalCOFINS.toFixed(2)}, Informado: ${(dados.totais.vCOFINS || 0).toFixed(2)}`,
        );
      }

      if (Math.abs((dados.totais.vTotTrib || 0) - totalTributos) > tolerancia) {
        this.erros.push(
          `Total de tributos divergente. Calculado: ${totalTributos.toFixed(2)}, Informado: ${(dados.totais.vTotTrib || 0).toFixed(2)}`,
        );
      }
    }
  }
  ...
  validarConformidadeSefaz(dados) {
    ...
    if (
      !dados.tipoEmissao ||
      ![1, 2, 3, 4, 5, 6, 7, 8, 9].includes(dados.tipoEmissao)
    ) {
    ...
    if (
      dados.processoEmissao === undefined ||
      ![0, 1, 2, 3].includes(dados.processoEmissao)
    ) {
  ...
  validarXmlResposta(xmlString) {
    try {
      if (!xmlString || typeof xmlString !== "string")
        return { valido: false, erros: ["Conteúdo inválido"], avisos: [] };
      if (!xmlString.includes("<?xml"))
        return { valido: false, erros: ["Cabeçalho XML ausente"], avisos: [] };
      const wellFormed = /<[^>]+>/.test(xmlString) && /<\/[A-Za-z]/.test(xmlString);
      return { valido: wellFormed, erros: wellFormed ? [] : ["XML possivelmente malformado"], avisos: [] };
    } catch (e) {
