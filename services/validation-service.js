...
class ValidationService {
    ...
    validarCNPJ(cnpj) {
        ...
        if (/^(\d)\1+$/.test(cnpjLimpo)) {
            return { valido: false, erro: 'CNPJ inválido - todos os dígitos são iguais' };
        }
    ...
    validarCPF(cpf) {
        ...
        if (/^(\d)\1+$/.test(cpfLimpo)) {
            return { valido: false, erro: 'CPF inválido - todos os dígitos são iguais' };
        }
    ...
    validarInscricaoEstadual(ie, uf) {
        ...
        return { 
            ...
            erro: ieLimpa.length < 8 || ieLimpa.length > 14 ? 'Inscrição Estadual com tamanho inválido' : null
        };
    }
