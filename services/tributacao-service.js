...
class TributacaoService {
  ...
  async calcularImpostos(item, emitente, destinatario, configuracao = {}) {
    try {
      ...
      const pis = await this.calcularPIS(
        item,
        emitente,
        valorLiquido,
        regimeTributario,
      );
      const cofins = await this.calcularCOFINS(
        item,
        emitente,
        valorLiquido,
        regimeTributario,
      );
      const iss = await this.calcularISS(
        item,
        emitente,
        destinatario,
        valorLiquido,
      ...
      // Calcular totais
      const totalImpostos =
        icms.valor + ipi.valor + pis.valor + cofins.valor + iss.valor;
      ...
      await logService.logErro("tributacao_calcular", error, {
        item: item.codigo,
      });
  ...
  async calcularICMS(item, emitente, destinatario, valorBase) {
    try {
      ...
      if (ufOrigem === ufDestino) {
        ...
        aliquota =
          this.aliquotasICMS.interestadual[ufOrigem]?.[ufDestino] || 12;
      }
  ...
  async calcularISS(item, emitente, destinatario, valorBase) {
    try {
      ...
      const municipio =
        destinatario?.endereco?.municipio || emitente.endereco?.municipio;
  ...
  determinarRegimeTributario(emitente) {
    ...
    switch (crt) {
      ...
      case "2":
        return "simples_nacional_excesso";
      case "3":
        return "lucro_real";
      default:
  ...
  verificarProdutoIndustrializado(ncm, item) {
    ...
    const ncmsIndustrializados = [
      ...
      "85",
      "87",
      "90", // Exemplos: máquinas, equipamentos, veículos, instrumentos
    ];

    const prefixoNCM = ncm.substring(0, 2);
    return (
      ncmsIndustrializados.includes(prefixoNCM) || item.industrializado === true
    );
  }
  ...
  gerarObservacoesFiscais(dados) {
    ...
    if (dados.icms.valor > 0) {
      observacoes.push(
        `ICMS: R$ ${dados.icms.valor.toFixed(2)} (${dados.icms.aliquota}%) - ${dados.icms.situacao}`,
      );
    }

    if (dados.ipi.valor > 0) {
      observacoes.push(
        `IPI: R$ ${dados.ipi.valor.toFixed(2)} (${dados.ipi.aliquota}%) - ${dados.ipi.situacao}`,
      );
    }

    if (dados.pis.valor > 0) {
      observacoes.push(
        `PIS: R$ ${dados.pis.valor.toFixed(2)} (${dados.pis.aliquota}%) - ${dados.pis.regime}`,
      );
    }

    if (dados.cofins.valor > 0) {
      observacoes.push(
        `COFINS: R$ ${dados.cofins.valor.toFixed(2)} (${dados.cofins.aliquota}%) - ${dados.cofins.regime}`,
      );
    }

    if (dados.iss.valor > 0) {
      observacoes.push(
        `ISS: R$ ${dados.iss.valor.toFixed(2)} (${dados.iss.aliquota}%) - ${dados.iss.municipio}`,
      );
    }

    // Informações sobre carga tributária
    const percentualCarga = (
      (dados.totalImpostos / dados.valorLiquido) *
      100
    ).toFixed(2);
    ...
    observacoes.push(
      `Regime tributário: ${dados.regimeTributario.replace("_", " ").toUpperCase()}`,
    );
    ...
    if (dados.ibs.valor > 0 || dados.cbs.valor > 0) {
      ...
      if (dados.ibs.valor > 0) {
        observacoes.push(
          `IBS: R$ ${dados.ibs.valor.toFixed(2)} (${dados.ibs.aliquota}%)`,
        );
      }
      if (dados.cbs.valor > 0) {
        observacoes.push(
          `CBS: R$ ${dados.cbs.valor.toFixed(2)} (${dados.cbs.aliquota}%)`,
        );
      }
    }
