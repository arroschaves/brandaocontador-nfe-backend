...
class EncryptionService {
  constructor() {
    ...
    if (!this.encryptionKey) {
      throw new Error('❌ ERRO CRÍTICO: ENCRYPTION_KEY não está configurado! Gere com: openssl rand -hex 32');
    }
    
    // Converter hex para buffer
    if (this.encryptionKey.length !== 64) {
      throw new Error('❌ ERRO CRÍTICO: ENCRYPTION_KEY deve ter 64 caracteres hexadecimais (32 bytes)');
    }
  ...
  decrypt(encryptedData) {
    try {
      ...
      throw new Error('Falha na descriptografia de dados - chave incorreta ou dados corrompidos');
    }
  }
