...
class AuthService {
  ...
  async register(dadosUsuario) {
    try {
      ...
      const emailExistente = await database.buscarUsuarioPorEmail(
      ...
      if (dadosUsuario.documento) {
        const documentoExistente = await database.buscarUsuarioPorDocumento(
          dadosUsuario.documento,
        );
        if (documentoExistente) {
          throw new Error(
            `${(dadosUsuario.tipoCliente || "cpf").toUpperCase()} já cadastrado`,
          );
        }
      }
